...
// For areebb.com domain with OAuth2/Modern Auth
...
// OAuth2 configuration (for Modern Auth)
const OAUTH_CLIENT_ID = process.env.OAUTH_CLIENT_ID || '';
const OAUTH_CLIENT_SECRET = process.env.OAUTH_CLIENT_SECRET || '';
const OAUTH_REFRESH_TOKEN = process.env.OAUTH_REFRESH_TOKEN || '';
const OAUTH_TENANT_ID = process.env.OAUTH_TENANT_ID || 'common';

// Create transporter - supports both basic auth and OAuth2
...
function getLogoBase64(): string {
  try {
    const logoPath = join(process.cwd(), 'attached_assets', 'Areeb-01_1768386546804.png');
